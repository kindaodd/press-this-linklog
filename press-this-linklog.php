 ...
 * 
 * This bookmarklet should be used in conjunction with Jonathan Penn's
 * WordPress Linked List plugin:
 * http://github.com/jonathanpenn/wordpress-linked-list-plugin
 *
 * This adds a field to control the outgoing url of linklog items, using a
 * custom field called "linked_list_url". Along with various UI improvements.
 *
 *
 * @package WordPress
...
<style type="text/css">
	.ac_results {border:1px solid #ddd; border-top: none; margin-top: -1px;}
	.ac_results li {border-top: 1px solid #fff;}
</style>
</head>
<body class="press-this" onload="var URLbox = document.getElementById('URL'); var postTitle = document.getElementById('title'); if (URLbox.value == '') {URLbox.style.color = '#999';
...
<div id="poststuff" class="metabox-holder">
	<div id="side-info-column">
		<div class="sleeve">
			<h1 id="viewsite"><a class="button" href="<?php echo get_option('home'); ?>/" target="_blank">Go to <?php bloginfo('name'); ?> &rarr;</a></span></h1>
				...
				<div class="inside">
					<p>
						...
						<?php if ( current_user_can('publish_posts') ) { ?>
							<input class="button-primary" style="float: right;" type="submit" name="publish" value="<?php esc_attr_e('Publish') ?>" id="publish" />
						<?php } else { ?>
			...
			<div id="tagsdiv-post_tag" class="stuffbox" >
				...
				<div class="inside">
					<div class="tagsdiv" id="post_tag">
						<p class="jaxtag">
							<label class="screen-reader-text" for="newtag" style="display: none;"><?php _e('Post Tags'); ?></label>
							<input type="hidden" name="tax_input[post_tag]" class="the-tags" id="tax-input[post_tag]" value="" />
							<span class="ajaxtag" style=" display:none;">
								<input type="text" name="newtag[post_tag]" class="newtag form-input-tip" style="outline: none; padding: 3px;" size="13" autocomplete="off" value="<?php esc_attr_e('Add new tag'); ?>" />
								<input type="button" style="float: right;" class="button tagadd" value="<?php esc_attr_e('Add'); ?>" tabindex="3" />
							</span>
						</p>
						<div class="tagchecklist"></div>
					</div>
					<p class="tagcloud-link"><a href="#titlediv" class="tagcloud-link" id="link-post_tag"><?php _e('Toggle tag cloud'); ?></a></p>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="posting">
		...
		<div id="titlediv">
			<div class="titlewrap">
				<input name="title" id="title" title="Post title" class="text" value="<?php echo esc_attr($title);?>" onfocus="if (this.value != '' && this.value != 'Post Title') { this.style.color = '#000'; this.style.fontStyle = 'normal'; } else if (this.value == 'Post Title') {this.style.color = '#000'; this.value = ''; this.style.fontStyle = 'normal';}" onblur="if (this.value == '') { this.style.color = '#999'; this.style.fontStyle = 'italic'; this.value = 'Post Title'; } else { this.style.fontStyle = 'normal'; }"/>
			</div>
		</div>
...
<img alt="<?php _e('Insert an Image'); ?>" src="images/media-button-image.gif"/></a>
			...
			<div class="editor-container">
				<span style="border-bottom: 1px solid #ccc; padding: 9px 0; display: block;">
				<input name="URL" type="text" id="URL" title="External URL" style="border: none; color: #999; padding: 0 5px; outline: none; width: 97%;" value="<?php echo $url; ?>" onfocus="if (this.value != '' && this.value != 'External URL (optional)') { this.style.color = '#000'; this.style.fontStyle = 'normal'; } else if (this.value == 'External URL (optional)') {this.style.color = '#000'; this.value = ''; this.style.fontStyle = 'normal';}" onblur="if (this.value == '') { this.style.color = '#999'; this.style.fontStyle = 'italic'; this.value = 'External URL (optional)'; } else { this.style.color = '#999'; this.style.fontStyle = 'normal'; }"/>
				</span>
				
				<textarea name="content" id="content" style="color: #000 !important; margin-top: 5px; outline: none; resize: none; width: 99%;" class="mceEditor" rows="15">
<?php if ($selection) echo wp_richedit_pre(htmlspecialchars_decode($selection)); ?>
<?php if ($url) { echo '<p>'; if($selection) _e('via '); echo "<a href='$url'>$title</a>."; echo '</p>'; } ?></textarea>
			</div>
		</div>
	</div>
</div>
